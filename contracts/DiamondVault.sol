...
// ═══════════════════════════════════════════════════════════════
//  AntVault — 蚁群算力分红合约 v6
//  BSC Mainnet
//
//  新增：WBNB 自动 unwrap
//  · receiveMainRewardWBNB(amount)  — 代币合约转入 WBNB 作为主分红
//  · receiveDiamondRewardWBNB(amt)  — 代币合约转入 WBNB 作为王者分红
//  · receiveBuybackBurnWBNB(amount) — 代币合约转入 WBNB 作为回购销毁
//  · unwrapAndFundMain() / unwrapAndFundDia() — 手动 unwrap 合约内的 WBNB
//  合约收到 WBNB 后自动调用 IWBNB.withdraw() 换成原生 BNB 再分红
// ═══════════════════════════════════════════════════════════════
...
// WBNB 标准接口（BSC 主网 WBNB）
interface IWBNB {
    function withdraw(uint256 amount) external; // unwrap WBNB → BNB
...
contract AntVault is Ownable, ReentrancyGuard {
    ...
    event WBNBUnwrapped(uint256 amount, string rewardType);
    ...
    /// @dev 从调用方 transferFrom WBNB → unwrap → 返回得到的 BNB 数量
    function _pullAndUnwrap(uint256 wbnbAmount) internal returns (uint256) {
        require(wbnbAmount > 0, "Zero amount");
        // 1. 把 WBNB 从调用方拉到本合约
        bool ok = IWBNB(WBNB).transferFrom(msg.sender, address(this), wbnbAmount);
        require(ok, "WBNB transferFrom failed");
        // 2. unwrap WBNB → 原生 BNB（会触发 receive()）
    ...
    // ═══════════════════════════════════════════════════════════
    //  接收分红 — WBNB 版（代币合约发 WBNB 时调用）
    //  调用方需先 approve(vaultAddress, amount) WBNB
    // ═══════════════════════════════════════════════════════════

    /// @notice 代币合约将 WBNB 作为主分红转入（自动 unwrap）
    function receiveMainRewardWBNB(uint256 wbnbAmount) external nonReentrant {
        uint256 bnb = _pullAndUnwrap(wbnbAmount);
        mainRewardPool += bnb;
        emit WBNBUnwrapped(bnb, "main");
        emit MainRewardReceived(bnb);
        if (block.timestamp >= lastMainDistributeTime + MAIN_REWARD_INTERVAL)
            _distributeMain();
    }

    /// @notice 代币合约将 WBNB 作为王者分红转入（自动 unwrap）
    function receiveDiamondRewardWBNB(uint256 wbnbAmount) external nonReentrant {
        uint256 bnb = _pullAndUnwrap(wbnbAmount);
        diaRewardPool += bnb;
        emit WBNBUnwrapped(bnb, "diamond");
        emit DiaRewardReceived(bnb);
        if (block.timestamp >= lastDiaDistributeTime + DIAMOND_REWARD_INTERVAL)
            _distributeDiamond();
    }

    /// @notice 代币合约将 WBNB 作为回购销毁转入（自动 unwrap 后回购）
    function receiveBuybackBurnWBNB(uint256 wbnbAmount) external nonReentrant {
        uint256 bnb = _pullAndUnwrap(wbnbAmount);
        emit WBNBUnwrapped(bnb, "buyback");
        emit BuybackReceived(bnb);
        _doBuybackBurn(bnb);
    }

    // ═══════════════════════════════════════════════════════════
    //  手动 unwrap（合约内已有 WBNB 时使用）
    // ═══════════════════════════════════════════════════════════

    /// @notice 把合约内的全部 WBNB 余额 unwrap 后加入主分红池
    function unwrapAndFundMain() external onlyOwner nonReentrant {
        uint256 bal = IWBNB(WBNB).balanceOf(address(this));
        require(bal > 0, "No WBNB");
        uint256 before = address(this).balance;
        IWBNB(WBNB).withdraw(bal);
        uint256 received = address(this).balance - before;
        mainRewardPool += received;
        emit WBNBUnwrapped(received, "main");
        emit MainRewardReceived(received);
    }

    /// @notice 把合约内的全部 WBNB 余额 unwrap 后加入王者分红池
    function unwrapAndFundDia() external onlyOwner nonReentrant {
        uint256 bal = IWBNB(WBNB).balanceOf(address(this));
        require(bal > 0, "No WBNB");
        uint256 before = address(this).balance;
        IWBNB(WBNB).withdraw(bal);
        uint256 received = address(this).balance - before;
        diaRewardPool += received;
        emit WBNBUnwrapped(received, "diamond");
        emit DiaRewardReceived(received);
    }
    ...
    function claim() external nonReentrant {
        ...
        if (onChain < u.totalBalance) { _penalizeUser(msg.sender); return; }
    ...
    function getUserInfo(address account) external view returns (
        uint256 totalBalance_, uint8 level_, uint256 oldestHeldHours_,
        uint256 power_, uint256 pendingMain_, uint256 pendingDia_,
        uint256 totalClaimed_, uint256 positionCount_
    ) {
        UserInfo storage u = users[account];
        ...
        uint256 mainInc  = power_ > 0 ? (power_ * (mainAccPerPower - u.mainRewardDebt)) / PRECISION : 0;
    ...
    function getUserPositions(address account) external view returns (
        uint256[] memory amounts_, uint256[] memory startTimes_, uint256[] memory heldHours_
    ) {
        Position[] storage pos = users[account].positions;
    ...
    function getLevelInfo(address account) external view returns (
        uint8 level_, string memory levelName_,
        uint256 multiplier_, uint256 heldHours_, uint256 nextLevelHours_
    ) {
        level_          = _calcLevel(account);
    ...
    function getGlobalStats() external view returns (
        uint256 totalPower_, uint256 mainPool_, uint256 diaPool_,
        uint256 totalMainDist_, uint256 totalDiaDist_, uint256 totalBuyback_,
        uint256 activeUsers_, uint256 nextMainDistTime_, uint256 nextDiaDistTime_,
    ...
    function getLevelTable() external pure returns (
        uint256[10] memory minHours_, uint256[10] memory multipliers_
    ) {
    ...
    // 紧急提取误转入的 WBNB
    function emergencyWithdrawWBNB() external onlyOwner {
        uint256 bal = IWBNB(WBNB).balanceOf(address(this));
        require(bal > 0, "No WBNB");
        IWBNB(WBNB).withdraw(bal);
